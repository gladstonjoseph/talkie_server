    ...
    jwt.verify(token, process.env.JWT_SECRET || 'your_super_secret_key_that_should_be_long_and_random', async (err, decoded) => {
      if (err) {
        if (err.name === 'TokenExpiredError') {
          ...
          // Decode the expired token to get the appInstanceId
          const decodedToken = jwt.decode(token);
          if (decodedToken && decodedToken.appInstanceId) {
            const { appInstanceId } = decodedToken;
            try {
              console.log(`Attempting to delete expired app instance: ${appInstanceId}`);
              await pool.query('DELETE FROM app_instances WHERE app_instance_id = $1', [appInstanceId]);
              console.log(`Successfully deleted expired app instance: ${appInstanceId}`);
            } catch (dbErr) {
              console.error(`Error deleting expired app instance ${appInstanceId} from database:`, dbErr);
            }
          } else {
              console.error('Could not decode appInstanceId from expired token.');
          }
      ...
      const { userId, appInstanceId } = decoded;

      // Check if app_instance_id exists and belongs to the correct user
      const appInstanceResult = await pool.query('SELECT * FROM app_instances WHERE app_instance_id = $1', [appInstanceId]);

      if (!appInstanceResult.rows.length) {
        return next(new Error('APP_INSTANCE_NOT_FOUND'));
      }

      const appInstance = appInstanceResult.rows[0];

      if (appInstance.global_user_id !== decoded.userId) {
        return next(new Error('APP_INSTANCE_USER_MISMATCH'));
      }

      // Check if last_connected is older than 13 days
      const lastConnectedDate = new Date(appInstance.last_connected);
      const thirteenDaysAgo = new Date(Date.now() - 13 * 24 * 60 * 60 * 1000);

      if (lastConnectedDate < thirteenDaysAgo) {
        return next(new Error('APP_INSTANCE_INACTIVE'));
      }

      // Update last_connected timestamp
      await pool.query('UPDATE app_instances SET last_connected = NOW() WHERE app_instance_id = $1', [appInstanceId]);

      socket.user = { id: userId, appInstanceId };
      next();
    });
  } catch (err) {
